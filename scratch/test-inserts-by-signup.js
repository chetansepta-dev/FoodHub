...
async function run() {
  const email = `customer_new_${Date.now()}@gmail.com`;
  ...
  if (signUpError) {
    ...
    // If sign up fails due to rate limit, we can't test authenticated inserts, but we can test inserts that don't need auth (if any) or check errors
    return;
  }
  ...
  console.log('\n2. Testing PROFILES with avatarUrl & role:');
