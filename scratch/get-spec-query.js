...
async function run() {
  const targetUrl = `${url}/rest/v1/?apikey=${key}`;
  ...
  try {
    ...
    if (spec.definitions && spec.definitions.food_items) {
      ...
      console.log('food_items properties types:', spec.definitions.food_items.properties);
    }
    if (spec.definitions && spec.definitions.profiles) {
      console.log('profiles properties:', spec.definitions.profiles.properties);
    }
    if (spec.definitions && spec.definitions.orders) {
      console.log('orders properties:', spec.definitions.orders.properties);
    }
    if (spec.definitions && spec.definitions.order_items) {
      console.log('order_items properties:', spec.definitions.order_items.properties);
    }
  } catch (e) {
